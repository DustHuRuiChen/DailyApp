...
namespace DailyApp.API.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        /// <returns>-1:账号已经存在；1:注册成功；-99:未知错误 或 服务器繁忙</returns>
        [HttpPost]
        public IActionResult Reg(AccountInfoDTO accountInfoDTO)
        {
            ...
            {
                //1.账号是否存在(没有考虑高并发）
               var dbAccount = db.Accounts.Where(t => t.Account == accountInfoDTO.Account).FirstOrDefault();
                if(dbAccount != null)
                {
                    res.ResultCode = -1;//账号已存在
                    res.Msg = "对不起，该账号已经被使用了";
        ...
        /// <returns>登录信息 1-登录成功。-1-账号错误 -2-密码错误</returns>
        ///
        [HttpGet]
        public IActionResult Login(string account, string pwd)
        {
            ...
            try
            {
                var dbAccountInfo = db.Accounts.Where(t => t.Account == account).FirstOrDefault();
                ...
                res.Msg = "登录成功";
                res.ResultData = dbAccountInfo;//返回登录信息
